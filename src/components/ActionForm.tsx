...
import { Action, DamageAction } from '../types/action';

type Props = {
  params: DamageAction;
  setParams(val: DamageAction | ((stats: DamageAction) => DamageAction)): void;
};
const ActionForm: React.FC<Props> = ({ setParams, params }) => (
  <form
    ...
    onChange={(e) => {
      setParams((curr) => {
        ...
        updated[e.nativeEvent.target.id] = parseFloat(e.target.value as string);
  ...
  >
    <ActionSelect
      ...
      setAction={(action: Action) => setParams(action as DamageAction)}
    />
    ...
    <div className="flex flex-row gap-4 items-start">
      ...
      <div className="flex flex-col items-start">
        ...
        <input value={params.base_damage} type="number" id="base_damage" title="base_damage" />
      </div>
      <div className="flex flex-col items-start">
